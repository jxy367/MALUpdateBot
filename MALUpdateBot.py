...
def convert_anime_update_to_embed(user, update):
    ...
    embed.add_field(name="Status: " + statuses[int(update['status'])])
    if update['score'] != 0:
        embed.add_field(name="Score: " + str(update['score']))
    if update['tags'] != '':
        embed.add_field(name="Tags: " + update['tags'])

    if update['is_rewatching'] != 0:
        embed.add_field(name="Rewatched: " + str(update['is_rewatching']) + " times")
...
def convert_manga_update_to_embed(user, update):
    ...
    if update['is_rereading'] != 0:
        embed.add_field(name="Reread: " + str(update['is_rereading']) + " times")
    ...
    if 'num_read_chapters' in update and 'manga_num_chapters' in update:
        ...
        if read_chapters < num_chapters:
            embed.add_field(name="Chapters read: " + str(read_chapters) + "/" + str(num_chapters))
        else:
            embed.add_field(name="Chapters read: " + str(read_chapters))
